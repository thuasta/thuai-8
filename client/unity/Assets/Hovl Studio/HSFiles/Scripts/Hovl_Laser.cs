...
public class Hovl_Laser : MonoBehaviour
{
    ...
    public Vector3 CustomEndPosition;
    [Tooltip("启用后使用自定义的起点终点，否则使用原有逻辑")]
    ...
    void Update()
    {
        ...
        if (UseCustomPoints)
        {
            Debug.Log(CustomStartPosition);
            Debug.Log(CustomEndPosition);
            // 将本地坐标转换为世界坐标
            startPos = transform.parent.TransformPoint(CustomStartPosition);
            endPos = transform.parent.TransformPoint(CustomEndPosition);
            ...
            if (FlashEffect != null)
            {
                ...
                FlashEffect.transform.rotation = Quaternion.LookRotation(endPos - startPos);
            }
            ...
            HitEffect.transform.LookAt(endPos + (endPos - startPos).normalized);

            foreach (var ps in Effects) if (!ps.isPlaying) ps.Play();
            return;
        }
